namespace BrowserWeb
{
    ...
    public class Utilities
    {
        ...
        public static bool CopyToClipboard(DataObject oData)
        {
            ...
            {
                SDVPApplication.DoNotifyUser("Please disable any clipboard monitoring tools and try again.\n\n" + exception.Message, ".NET Framework Bug");
                return true;
            }
        }
        ...
        public static byte[] DeflaterCompress(byte[] writeData)
        {
            ...
            {
                SDVPApplication.DoNotifyUser("The content could not be compressed.\n\n" + exception.Message, "SDVP: Deflation failed");
                return writeData;
            }
        }
        ...
        public static byte[] DeflaterExpand(byte[] compressedData)
        {
            try
            {
                return DeflaterExpandInternal(Browser.bUseXceedDecompressForDeflate, compressedData);
            }
            catch (Exception exception)
            {
                SDVPApplication.DoNotifyUser("The content could not be decompressed.\n\n" + exception.Message, "SDVP: Inflation failed");
                return new byte[0];
            }
        }
        ...
        public static byte[] doUnchunk(byte[] writeData)
        {
            ...
            if ((!flag && Browser.bReportHTTPErrors) && !Browser.QuietMode)
            {
                SDVPApplication.DoNotifyUser("Chunked body did not terminate properly with 0-sized chunk.", "HTTP Protocol Violation");
            }
        ...
        internal static bool SDVPMeetsVersionRequirement(Assembly assemblyInput, string sWhatType)
        {
            ...
            int num = CompareVersions(customAttribute.RequiredVersion, Browser.SDVPVersionInfo);
            if (num > 0)
            {
                SDVPApplication.DoNotifyUser(string.Format("The {0} in {1} require SDVP v{2} or later. (You have v{3})\n\nPlease install the latest version of SDVP from http://www.SDVP2.com.\n\nCode: {4}", new object[] { sWhatType, assemblyInput.CodeBase, customAttribute.RequiredVersion, Browser.SDVPVersionInfo, num }), "Extension Not Loaded");
                return false;
            }
            return true;
        }
        ...
        [CodeDescription("Gets (via Headers or Sniff) the provided body's text Encoding. Returns Browser.oHeaderEncoding (usually UTF-8) if unknown. Potentially slow.")]
        public static Encoding getEntityBodyEncoding(HTTPHeaders oHeaders, byte[] oBody)
        {
            if (oHeaders == null)
            {
                return Browser.oHeaderEncoding;
            }
            ...
            Encoding oHeaderEncoding = Browser.oHeaderEncoding;
        ...
        internal static string GetFirstLocalResponse(string sFilename)
        {
            try
            {
                if (!Path.IsPathRooted(sFilename))
                {
                    ...
                    sFilename = Browser.GetPath("TemplateResponses") + str;
                    if (!System.IO.File.Exists(sFilename))
                    {
                        sFilename = Browser.GetPath("Responses") + str;
                    }
                }
            }
            catch (Exception)
        ...
        [CodeDescription("Gets (via Headers or Sniff) the Response Text Encoding. Returns Browser.oHeaderEncoding (usually UTF-8) if unknown. Potentially slow.")]
        ...
        public static byte[] GzipCompress(byte[] writeData)
        {
            ...
            {
                SDVPApplication.DoNotifyUser("The content could not be compressed.\n\n" + exception.Message, "SDVP: GZip failed");
                return writeData;
            }
        }
        ...
        public static byte[] GzipExpand(byte[] compressedData)
        {
            try
            {
                return GzipExpandInternal(Browser.bUseXceedDecompressForGZIP, compressedData);
            }
            catch (Exception exception)
            {
                SDVPApplication.DoNotifyUser("The content could not be decompressed.\n\n" + exception.Message, "SDVP: UnGZip failed");
                return new byte[0];
            }
        }
        ...
        internal static bool IsChunkedBodyComplete(Session m_session, MemoryStream oData, long iStartAtOffset, out long outStartOfLatestChunk, out long outEndOfEntity)
        {
            ...
            while (index < length)
            {
                ...
                if (!TryHexParse(sInput, out iOutput))
                {
                    ...
                    SDVPApplication.HandleHTTPError(m_session, flagViolation, true, true, "Illegal chunked encoding. '" + sInput + "' is not a hexadecimal number.");
                    return true;
                }
                if (iOutput == 0)
                {
        ...
        public static bool LaunchHyperlink(string sURL)
        {
            ...
            {
                SDVPApplication.DoNotifyUser("Your web browser is not correctly Browserured to launch hyperlinks.\n\nTo see this content, visit:\n\t" + sURL + "\n...in your web browser.\n\nError: " + exception.Message, "Error");
            }
            return false;
        }
        ...
        public static string ObtainOpenFilename(string sDialogTitle, string sFilter, string sInitialDirectory)
        {
            ...
            dialog.CustomPlaces.Add(Browser.GetPath("Captures"));
        ...
        public static bool RunExecutableAndWait(string sExecute, string sParams)
        {
            ...
            {
                SDVPApplication.DoNotifyUser("SDVP Exception thrown: " + exception.ToString() + "\r\n" + exception.StackTrace.ToString(), "ShellExecute Failed");
                return false;
            }
        }
