namespace BrowserWeb
{
    ...
    public class ServerPipe : BasePipe
    {
        ...
        {
            if (SDVPApplication.oDefaultClientCertificate != null)
            {
                return SDVPApplication.oDefaultClientCertificate;
            }
            X509Certificate certificate = null;
            if (System.IO.File.Exists(Browser.GetPath("DefaultClientCertificate")))
            {
                certificate = X509Certificate.CreateFromCertFile(Browser.GetPath("DefaultClientCertificate"));
                if ((certificate != null) && SDVPApplication.Prefs.GetBoolPref("SDVP.network.https.cacheclientcert", true))
                {
                    SDVPApplication.oDefaultClientCertificate = certificate;
                }
            }
            return certificate;
        }
        ...
        private X509CertificateCollection GetCertificateCollectionFromFile(string sClientCertificateFilename)
        {
            ...
            {
                sClientCertificateFilename = Utilities.EnsurePathIsAbsolute(Browser.GetPath("Root"), sClientCertificateFilename);
        ...
        internal void MarkAsAuthenticated(int clientPID)
        {
            ...
            int num = SDVPApplication.Prefs.GetInt32Pref("SDVP.network.auth.reusemode", 0);
