...
namespace BrowserWeb
{
    ...
    public class ClientPipe : BasePipe
    {
        ...
        private static bool _bWantClientCert = SDVPApplication.Prefs.GetBoolPref("SDVP.network.https.requestclientcertificate", false);
        private int _iProcessID;
        private string _sProcessName;
        internal static int _timeoutReceiveInitial = SDVPApplication.Prefs.GetInt32Pref("SDVP.network.timeouts.clientpipe.receive.initial", 0xea60);
        internal static int _timeoutReceiveReused = SDVPApplication.Prefs.GetInt32Pref("SDVP.network.timeouts.clientpipe.receive.reuse", 0x7530);
            ...
            : base(oSocket, "C")
        {
            try
            {
                ...
                if (Browser.bMapSocketToProcess)
                {
