namespace BrowserWeb
{
    ...
    internal class DNSResolver
    {
        ...
        private static int MSEC_DNS_CACHE_LIFETIME = SDVPApplication.Prefs.GetInt32Pref("SDVP.network.timeouts.dnscache", 0x249f0);

        static DNSResolver()
        {
            SDVPApplication.Janitor.assignWork(new SimpleEventHandler(DNSResolver.ScavengeCache), 0x7530);
        }
        ...
        private static IPAddress[] trimAddressList(IPAddress[] arrResult)
        {
            ...
            {
                if (!list.Contains(arrResult[i]) && (Browser.bEnableIPv6 || (arrResult[i].AddressFamily == AddressFamily.InterNetwork)))
                {
