namespace BrowserWeb
{
    ...
    internal class WinINETConnectoids
    {
            ...
            foreach (string str in RASInfo.GetConnectionNames())
            {
                if (!this._oConnectoids.ContainsKey(str))
                {
                    ...
                    if (info != null)
                    {
                        ...
                        if ((info.sHttpProxy != null) && info.sHttpProxy.Contains(Browser.sSDVPListenHostPort))
                        {
        ...
        {
            string sHookConnectionNamed = Browser.sHookConnectionNamed;
        ...
        internal bool HookConnections(WinINETProxyInfo oNewInfo)
        {
            ...
            {
                if ((Browser.bHookAllConnections || (connectoid.sConnectionName == Browser.sHookConnectionNamed)) && oNewInfo.SetToWinINET(connectoid.sConnectionName))
                {
                    flag = true;
